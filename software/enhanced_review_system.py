...
class PanicBehaviorDetector(QObject):
    ...
    def _detect_running_behavior(self, camera_id: str, frame: np.ndarray, 
        ...
        for detection in people_detections:
            person_id = f"{detection['center'][0]}_{detection['center'][1]}"
            current_center = detection['center']
            current_time = time.time()
            
            if person_id in tracker['person_tracks']:
                prev_data = tracker['person_tracks'][person_id]
            ...
            # Update tracking
            tracker['person_tracks'][person_id] = {
